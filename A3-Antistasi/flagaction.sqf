...
switch _tipo do
	...
	case "moveS": {_flag addAction ["Move this asset", "moveHQObject.sqf",nil,0,false,true,"","(true)"]};
	case "remove":
		{
		if (player == _flag) then
			{
	...
	case "buildHQ": {_flag addAction ["Build HQ here", {[] spawn buildHQ},nil,0,false,true,"","(isPlayer _this) and (_this == _this getVariable ['owner',objNull])",4]};
	case "seaport": {_flag addAction ["Buy Boat", {[vehSDKBoat] call buyBoat},nil,0,false,true,"","(isPlayer _this) and (_this == _this getVariable ['owner',objNull])",4]};
	...
	case "fuego":
		{
		fuego addAction ["Rest for 8 Hours", "skiptime.sqf",nil,0,false,true,"","(true)",4];
		fuego addAction ["Clear Nearby Forest", "clearForest.sqf",nil,0,false,true,"","(true)",4];
		fuego addAction ["On\Off Lamp", "onOffLamp.sqf",nil,0,false,true,"","(isPlayer _this)",4];
		fuego addAction ["I hate the fog", "[10,0] remoteExec [""setFog"",2]",nil,0,false,true,"","(true)",4];
		};
	...
	case "SDKFlag":
		{
		removeAllActions _flag;
		_flag addAction ["Unit Recruitment", {nul=[] execVM "Dialogs\unit_recruit.sqf";;},nil,0,false,true,"","(isPlayer _this) and (_this == _this getVariable ['owner',objNull])",4];
		_flag addAction ["Buy Vehicle", {nul = createDialog "vehicle_option";},nil,0,false,true,"","(isPlayer _this) and (_this == _this getVariable ['owner',objNull])",4];
		if (isMultiplayer) then
			{
			_flag addAction ["Personal Garage", {nul = [true] spawn garage},nil,0,false,true,"","(isPlayer _this) and (_this == _this getVariable ['owner',objNull])",4];
			_flag addAction ["Faction Garage", {nul = [false] spawn garage},nil,0,false,true,"","(isPlayer _this) and (_this == _this getVariable ['owner',objNull])",4];
			}
		else
			{
			_flag addAction ["Faction Garage", {nul = [false] spawn garage},nil,0,false,true,"","(isPlayer _this) and (_this == _this getVariable ['owner',objNull])",4]
